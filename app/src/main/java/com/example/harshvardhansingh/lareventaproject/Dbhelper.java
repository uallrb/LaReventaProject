...
public class Dbhelper extends SQLiteOpenHelper {
    ...
    public Boolean topdealsinsertServer(String name, String description, String url,  float price,String phone,String sellername,int itemold,String category)
    {
        ...
        long ret= db.insert("topdealsserver", null, contentValues);
        if(ret==-1)
        {
            return false;
        }
    ...
    public Cursor readtempoSno(int sno)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        Cursor res=db.rawQuery("select * from " + " " + "topdealsserver where no="+sno, null);

        return res;


    }
    public Cursor readtempoFromCategory(String category)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        Cursor res=db.rawQuery("select * from " + " " + "topdealsserver where category='"+category+"'", null);
    ...
    public Cursor searchfromname(String name,float price)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        Cursor res=db.rawQuery("select * from " + " " + "topdeals"+" "+"where name='"+name+"' AND price="+price, null);
        return res;


    }
    public Cursor searchfromnameServer(String name,float price)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        Cursor res=db.rawQuery("select * from " + " " + "topdealsserver"+" "+"where name='"+name+"' AND price="+price, null);
        return res;
    ...
    public Cursor readtempo1()
    {
        SQLiteDatabase db=this.getWritableDatabase();
        Cursor res=db.rawQuery("select * from " + " " + "credentials", null);
